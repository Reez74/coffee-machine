  ...
  bills[i].onmousedown = function(event){
    ...
    document.onmouseup = function(){
      ...
      if (bill_top>bill_acc_top && bill_right<bill_acc_right && bill_left>bill_acc_left && bill_top<bill_acc_bottom){
        ...
        let audio = new Audio("audio/banknote.mp3").play();
        audio.play();
      }
    }
  ...
  if (money.value >= cost){
    ...
    let timerId = setInterval(function(){
      ...
      if (w==105){
        ...
        displayInfo.innerHTML = `<i class="fas fa-hourglass-start fa-2x"></i> Ожидайте...`;
      }else if (w<75){
        displayInfo.innerHTML = `<i class="fas fa-hourglass-half fa-2x"></i> Ожидайте...`;
      }else{
        displayInfo.innerHTML = `<i class="fas fa-hourglass-end fa-2x"></i> Ожидайте...`;
      }
    },100);
    
    let audio = new Audio("audio/g_coffee.mp3").play();
    audio.play();
  }
  else
    displayInfo.innerText = "Пополните баланс!";
}
...
function getChange(num,isRecursion=false){
  ...
  if (coin>0){
    ...
    let audio = new Audio("audio/g_change.mp3").play();
    audio.play();
  }
}
